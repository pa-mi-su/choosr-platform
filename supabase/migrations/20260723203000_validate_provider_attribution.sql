...
begin
  ...
  if exists (
    ...
    where jsonb_typeof(item) <> 'object'
      ...
      or (item ? 'attribution' and (
        jsonb_typeof(item->'attribution') is distinct from 'object'
        or nullif(btrim(item#>>'{attribution,label}'), '') is null
        or length(item#>>'{attribution,label}') > 160
        or nullif(btrim(item#>>'{attribution,url}'), '') is null
        or length(item#>>'{attribution,url}') > 2048
        or item#>>'{attribution,url}' !~ '^https://'
      ))
  ) then
