...
begin
  ...
  from (
    select
      ...
      md5(p_session_id::text || ':' || cr.item_id) as tie_breaker
    from public.choice_rankings cr
...
comment on function public.submit_rankings(uuid, integer, text[]) is
  'Requires every accepted choice when there are three or fewer, otherwise a private top three. Exact score/rank ties use a deterministic room-specific hash so neither participant nor deck position is favored.';
